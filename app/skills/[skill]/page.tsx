...
const JobListings: React.FC<JobListingsProps> = () => {
  const params = useParams<{ skill: string }>(); // Get title from route parameters
  const skill = params?.skill; // Use optional chaining to safely access title
  ...
  const formatSkill = useCallback((skill: string) => { // Wrap in useCallback
    return skill
      .replace(/-/g, ' ') // Replace hyphens with spaces
      .split(' ') // Split into words
      .filter(word => word.toLowerCase() !== 'jobs') // Remove the word 'jobs'
      .map(word => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase()) // Capitalize the first letter of each word
      .join(' '); // Join the words back into a single string
  }, []); // Add dependencies if needed
  const formattedSkill = formatSkill(skill); // Format the title
  const fetchJobs = useCallback(async () => {
    if (typeof window === 'undefined' || !formatSkill) return; // Check jobType here
    setIsLoading(true);
    ...
    try {
      ...
      if (formattedSkill) {
        query = query.or(`title.ilike.%${formattedSkill}%,skills.ilike.%${formattedSkill}%`);
      }
  ...
  }, [formatSkill, formattedSkill, selectedSources, currentPage]); // Add jobType to dependencies
  ...
  return (
    <div className={`w-full max-w-7xl mx-auto px-4 md:px-6 lg:px-8 py-12`}>
      ...
      <h1 className="text-2xl font-bold mb-4">
        {`Jobs for ${formattedSkill}`}
      </h1>

      <p className="mb-8">
            Uncover the latest job openings for people skilled with {formattedSkill} shared on LinkedIn and Twitter (X) that often fly under the radar of traditional job boards. <br />
