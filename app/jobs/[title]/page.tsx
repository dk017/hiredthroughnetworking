...
const JobListings: React.FC<JobListingsProps> = () => {
  const params = useParams<{ title: string }>(); // Get title from route parameters
  const title = params?.title; // Use optional chaining to safely access title
  ...
  const formatLocationAndTitle = (title: string) => {
    ...
    return { formattedTitle, formattedLocation };
  };
  ...
  const { formattedTitle, formattedLocation } =  formatLocationAndTitle(title);
  // Format the title
  const fetchJobs = useCallback(async () => {
    if (typeof window === 'undefined' || !formattedTitle) return; // Check jobType here
    setIsLoading(true);
    try {
        let query = supabase
        .from('jobs_nh')
        ...
        if (formattedTitle) {
            query = query.ilike('title',`%${formattedTitle}%`);
        }
  ...
  }, [formattedTitle, formattedLocation,  selectedSources, currentPage]); // Add jobType to dependencies
  ...
  return (
    <div className={`w-full max-w-7xl mx-auto px-4 md:px-6 lg:px-8 py-12`}>
      ...
      <h1 className="text-2xl font-bold mb-4">
        {`${formattedTitle} Jobs in ${formattedLocation}`}
      </h1>

      <p className="mb-8">
            Uncover the latest job openings for {formattedTitle} in {formattedLocation} shared on LinkedIn and Twitter (X) that often fly under the radar of traditional job boards. <br />
