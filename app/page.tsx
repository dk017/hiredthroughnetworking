...
import ExponentAffiliate from "./components/ExponentAffiliate";
...
const JobBoard: React.FC = () => {
  ...
  >([]); // Replace JobType with your actual job type
  ...
  const [selectedExperience, setSelectedExperience] = useState<string | null>(
    null
  ); // New state for experience filter
  const [selectedJob, setSelectedJob] = useState<{
    ...
    category: string;
    is_linkedin: boolean;
  } | null>(null); // New state for selected job
  ...
  const fetchJobs = useCallback(async () => {
    ...
    try {
      ...
      if (selectedLocations.length > 0 || inputValueLocation.trim()) {
        const locationsToQuery =
          selectedLocations.length > 0
            ? selectedLocations
                .map((location) => `location.ilike.%${location}%`)
                .join(",")
      ...
      if (
        ...
        inputValueSkill.trim() ||
        inputValueCompany.trim() ||
        inputValueLocation.trim()
      ) {
        query.or(
          selectedSources
            .map((source) =>
              source === "twitter"
                ? "is_linkedin.eq.false"
                : "is_linkedin.eq.true"
            )
            .join(",")
        );
  ...
  const handleKeyDownForLocation = (
      ...
      <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6`}>
        {displayedJobs.map((job, index) => (
          <React.Fragment key={`${job.id}-${index}`}>
                  ...
                  {Array.isArray(job.skills)
                    ? job.skills.map((skill) => (
                        <Badge
                          ...
                          variant="outline"
                          className={
                            "inline-block bg-muted text-muted-foreground px-2 py-1 rounded-full"
                          }
                        >
                          {skill}
                        </Badge>
            ...
            {(index + 1) % 20 === 0 && <ExponentAffiliate />}
          </React.Fragment>
        ))}
      </div>
